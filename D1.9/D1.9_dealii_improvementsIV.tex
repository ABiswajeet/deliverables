...
\begin{documentControl}
    ...
    \addVersion{0.3}{July 31}{Luca Heltai}{Refinement on UNIPI part}
    \addVersion{1.0}{July 31}{Luca Heltai}{Final version for submission}
\end{documentControl}
...
The purpose of this document is to summarize and assess the \texttt{deal.II}-related improvements completed during the fourth semester of the dealii-X project. The report describes the mathematical motivation, software implementation, software-delivery status, and available numerical or performance results for each contribution. Particular attention is given to developments that improve exascale readiness by reducing memory consumption, increasing computational intensity, supporting heterogeneous hardware, improving distributed-memory scalability, or coupling models and solvers with different dimensionality and computational characteristics.

The document also records progress toward reusable and openly available capabilities rather than isolated application-specific implementations. This includes software integrated directly into \texttt{deal.II}, external libraries exposed through stable \texttt{deal.II} interfaces, and application-level research software released in public repositories with documentation, tests, benchmark data, and continuous-integration support. These developments are intended to become general building blocks for several dealii-X lighthouse applications, including simulations of the respiratory and cardiovascular systems, cardiac electrophysiology, brain and liver mechanics, and other coupled organ models.

\subsection{Structure of the Document}

Section~\ref{sec:section2} presents the UNIPI contribution on an implicit hybridizable discontinuous Galerkin solver for geometrically reduced flow models on metric graphs, including the public release of the \texttt{metric-flow-x} research software. Section~\ref{sec:section3} reports the INPT activities on block low-rank approximations and mixed-precision algorithms in MUMPS. The following sections describe the SISSA developments in polygonal and unfitted discretizations and the RUB work on accelerator-oriented finite element kernels and portable multigrid methods. Section~\ref{sec:section6} presents the UNITOV integration of PSBLAS and AMG4PSBLAS into \texttt{deal.II}, together with weak- and strong-scaling studies on MareNostrum~5 for a Poisson benchmark and a patient-specific cardiac electrophysiology problem. The report concludes with a synthesis of the advances and their relevance to the exascale objectives of dealii-X.
\newpage
